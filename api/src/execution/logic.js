  ...
  try {
    ...
    const result = await db.query(
      `INSERT INTO execution_steps (
          id, execution_id, user_id, org_id, step_type, status, reasoning, output, error, started_at
       )
...
async function runAnalysisGoal(payload, executionId, cb) {
  ...
  await recordStep(executionId, "analysis", "running", "Analyzing provided text...");
  const result = { length: text.length, wordCount: text.split(" ").length };
  await recordStep(executionId, "analysis", "completed", "Analysis complete.", result);
...
async function runAutomationGoal(payload, executionId, cb) {
  ...
  for (const stepName of payload.steps) {
    await recordStep(executionId, "automation_task", "running", `Initiating task: ${stepName}`);
    await new Promise(r => setTimeout(r, 800));
    await recordStep(executionId, "automation_task", "completed", `Finished: ${stepName}`, { task: stepName, status: "success" });
  }
...
async function runAiPlanGoal(payload, executionId, cb) {
  ...
  try {
    ...
    await recordStep(executionId, "ai_plan", "completed", "AI plan generated.", result);
