  ...
  return (
    <div className="p-6 space-y-10 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <InfoCard
          label="System Health"
          value={health?.status === "ok" ? "✅ Healthy" : "⚠️ Issues"}
          accent="from-green-400 to-green-600"
        />
        <InfoCard
          label="Database"
          value={health?.db ? "Connected" : "Disconnected"}
            ...
            <table className="min-w-full border-separate border-spacing-y-2">
              ...
              <tbody>
                {executions.slice(0, 5).map((exec, idx) => (
                  <tr
                    key={exec.id}
                    className="group bg-white dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900 transition-colors rounded-lg shadow-sm"
                  >
...
function InfoCard({ label, value, accent }) {
  return (
    <div className="relative bg-white dark:bg-gray-800 p-6 rounded-xl shadow group overflow-hidden transition-all duration-300 hover:scale-[1.03] hover:shadow-xl">
      ...
      />
      <div className="text-sm text-gray-500 dark:text-gray-400 font-medium mb-1 z-10 relative">
        {label}
      </div>
